...
export const repayment_amount_ = ({ i_in, principal_in, term_in }) => {
  if (i({ i_in }) == 0) return principal({ principal_in }) / term({ term_in });else
  return principal({ principal_in }) * i({ i_in }) / (1 - v_pow_term({ i_in, term_in }));
};
...
export const repayment_ = ({ year_in, term_in, i_in, principal_in }) => repayment_made({ year_in, term_in }) * repayment_amount({ i_in, principal_in, term_in });

export const balance_ = ({ year_in, principal_in, i_in, term_in }) => {
  if (year({ year_in }) == 0) return principal({ principal_in });else
  return balance({ principal_in, i_in, term_in, year_in: year({ year_in }) - 1 }) + interest({ year_in, principal_in, term_in, i_in }) - repayment({ year_in, term_in, i_in, principal_in });
};
